	...
	if ($result = mysqli_query($db, $sql)) {
		...
		while ($data = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
			echo '<div class="imgDiv" onmouseover="showOptions(this)" onmouseleave="hideOptions(this)">
				...
				<label class="switch">					
					<input type="checkbox" class="button" id="button'.$data["channelNumber"].'" name="'; if ($data["channelStatus"] == 1) echo 'ch'.$data["channelNumber"].'OFF"'; else echo 'ch'.$data["channelNumber"].'ON"'; echo '>	
